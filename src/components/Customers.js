class Customers extends React.Component {
  ...
  componentDidMount() {
    let storeCustomers = store.getState().customers;
    this.setState({ customers: storeCustomers });
    store.subscribe(() => {
      let search = store.getState().searchTerm;
      let storeCustomers = store.getState().customers;
      this.setState({ customers: storeCustomers, searchTerm: search });
    });
  }
  viewCustomer(cust) {
    ...
    return (
      <div className="page">
        <table className="table">
          ...
          <tbody id="search-customer-tbody">{tbody}</tbody>
        </table>
      </div>
    );
  }
